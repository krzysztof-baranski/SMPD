...
{
    QString fileName = QFileDialog::getOpenFileName(this,tr("Open TextFile"), "", tr("Texts Files (*.txt)"));
...
//    for (int i =0; i < 3; i++) {
    ...
    //    cv::waitKey(1000);
        ...
        replace( folderName.begin(), folderName.end(), '_', ' ');
        cout << "folderName " << folderName << endl;

        fd = countFourierDescriptors(allPoints, path);
...
//     std::vector<float> descriptors;
    ...
    float cLower (*min_element(descriptors.begin(), descriptors.end()));
    cout << "max " << cLower << endl;
    float cUpper (*max_element(descriptors.begin(), descriptors.end()));
    cout << "max " << cUpper << endl;

    for (int i=0; i< descriptors.size(); i++) {
        descriptors[i] = (descriptors[i] - cLower)/(cUpper - cLower);
        cout << descriptors[i] << endl;
    }
    cout << "descriptors " << descriptors.size() << endl;

    return descriptors;
//    classFouriersMap.insert(make_pair(folderName, descriptors));
}

void MainWindow::writeToFile (string folderName, QString filename) {
//    QFile file = folderName;

    if ( file.open(QIODevice::Append) )
    {
        QTextStream stream( &file );
        stream << QString::fromStdString(folderName);

        for(int i=0; i<fd.size(); i++) {
            stream << "," << QString::fromStdString(to_string(fd[i]));
        }
        stream << endl;
    }
    file.close();
